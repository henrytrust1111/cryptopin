...
const News: React.FC = () => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? articles.length - itemsToShow : prevIndex - itemsToShow
    );
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex >= articles.length - itemsToShow ? 0 : prevIndex + itemsToShow
    );
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-8">
      ...
      <div className="relative bg-gray-100 rounded-lg shadow-lg overflow-hidden p-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {articles.slice(currentIndex, currentIndex + itemsToShow).map((article, index) => (
